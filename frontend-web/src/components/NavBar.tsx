...
export function NavBar() {
  ...
  const links = (mobile = false) => sections.map((section) => <div key={section.label} className="mb-5">{(!collapsed || mobile) && <div className="mb-2 px-3 text-[10px] font-semibold tracking-[0.12em] text-slate-400">{section.label}</div>}<div className="space-y-1">{section.items.map((item) => { const Icon = item.icon; return <Link key={item.path} to={item.path} onClick={() => mobile && setMobileOpen(false)} title={!mobile && collapsed ? item.label : undefined} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${active(item.path) ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'} ${!mobile && collapsed ? 'justify-center' : ''}`}><Icon size={18} strokeWidth={active(item.path) ? 2.2 : 1.8} className={active(item.path) ? 'text-slate-900' : 'text-slate-400'} />{(!collapsed || mobile) && <span className="truncate">{item.label}</span>}</Link> })}</div></div>)
  return <>
    <aside className={`fixed inset-y-0 left-0 z-50 hidden flex-col border-r border-slate-200 bg-white transition-[width] duration-200 lg:flex ${collapsed ? 'w-[72px]' : 'w-60'}`}><div className="flex h-16 shrink-0 items-center border-b border-slate-100 px-4"><Link to="/" className="flex min-w-0 items-center gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-white shadow-sm"><WalletCards size={19}/></div>{!collapsed && <div><div className="text-sm font-bold tracking-tight text-slate-900">Finance Pro</div><div className="text-[9px] font-medium uppercase tracking-[0.16em] text-slate-400">Gestion financière</div></div>}</Link></div><nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4">{links()}<div className="mt-5 border-t border-slate-100 pt-4"><Link to="/organization" title={collapsed ? 'Paramètres' : undefined} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 hover:bg-slate-50 ${collapsed ? 'justify-center' : ''}`}><Settings size={18}/>{!collapsed && <span>Paramètres</span>}</Link></div></nav><div className="border-t border-slate-100 p-3"><div className={`flex items-center gap-3 rounded-xl bg-slate-50 p-2 ${collapsed ? 'justify-center' : ''}`}><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white">{initials}</div>{!collapsed && <div className="min-w-0 flex-1"><div className="truncate text-xs font-semibold text-slate-700">{user?.full_name}</div><button onClick={handleLogout} className="mt-0.5 text-[11px] text-slate-400 hover:text-red-600">Déconnexion</button></div>}</div></div></aside>
