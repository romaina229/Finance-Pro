...
export async function approveExpense(organizationId: string, expenseId: string, overrideBudgetControl = false) { const { data } = await api.post(`/organizations/${organizationId}/expenses/${expenseId}/approve`, { override_budget_control: overrideBudgetControl }); return data.data as Expense }
