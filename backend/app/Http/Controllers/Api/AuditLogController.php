...
class AuditLogController extends Controller
{
    ...
    {
        $query = AuditLog::query()->where('organization_id', $organization->id)->with('user:id,full_name')->latest('created_at');
        if ($request->filled('action')) $query->where('action', 'like', (string) $request->input('action').'.%');
        ...
        return response()->json([
            'data' => $logs->getCollection()->map(fn (AuditLog $log) => [
                ...
                'metadata' => $log->metadata, 'ip_address' => $log->ip_address, 'created_at' => $log->created_at,
