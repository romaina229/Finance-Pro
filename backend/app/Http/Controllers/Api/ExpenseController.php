...
class ExpenseController extends Controller
{
    ...
    {
        $query = $organization->expenses()->with(['project:id,name,code','category:id,name,code','paymentMethod:id,name,code','cashRegister:id,name,code','bankAccount:id,name,code','creator:id,full_name']);
        if ($request->filled('project_id')) $query->where('project_id',$request->project_id);
        if ($request->filled('status')) $query->where('status',$request->status);
        return response()->json(['data'=>$query->orderByDesc('expense_date')->get()]);
    }
    public function show(Request $request, Organization $organization, Expense $expense){abort_if($expense->organization_id!==$organization->id,404);return response()->json(['data'=>$expense->load(['project','category','paymentMethod','cashRegister','bankAccount','creator','approver'])]);}
    public function store(Request $request, Organization $organization){$validator=Validator::make($request->all(),$this->rules());if($validator->fails())throw new ValidationException($validator);$data=$validator->validated();$data['currency']=$data['currency']??$organization->default_currency;$data['amount_in_org_currency']=$data['amount'];$expense=$organization->expenses()->create([...$data,'created_by'=>$request->user()->id,'status'=>'draft']);return response()->json(['data'=>$expense->load(['project','category','paymentMethod','cashRegister','bankAccount'])],201);}
    public function update(Request $request, Organization $organization, Expense $expense){abort_if($expense->organization_id!==$organization->id,404);if(!in_array($expense->status,['draft','rejected']))return response()->json(['message'=>'Seule une dépense en brouillon ou rejetée peut être modifiée.'],422);$validator=Validator::make($request->all(),$this->rules(true));if($validator->fails())throw new ValidationException($validator);$expense->update($validator->validated());return response()->json(['data'=>$expense->fresh(['project','category','paymentMethod','cashRegister','bankAccount'])]);}
